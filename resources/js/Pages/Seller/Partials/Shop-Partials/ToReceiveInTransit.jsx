...
export default function ToReceiveInTransit({ toReceiveInTransitData }) {
    ...
    const submitToReceive = (e, id) => {
        e.preventDefault();
        setProcessingId(id);
        patch(route("seller.order.process.outForDelivery", { id }));
    };
    return (
        <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {toReceiveInTransitData.length == 0 ? (
                    ...
                    toReceiveInTransitData.map((item) => (
                        <div
                            key={item.id}
